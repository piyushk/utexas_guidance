...
namespace bwi_guidance_solver {
  ...
  void GuidanceModel::takeAction(const utexas_planning::State::ConstPtr& state_base,
    ...
    if (action.type != WAIT) {
      ...
      if (action.type == ASSIGN_ROBOT) {
        ...
        next_state.requests[action.request_id].assist_type = DIRECT_PERSON;
  ...
  void GuidanceModel::getColocatedRobotRequestIds(const State& state,
    ...
    for (int robot_id = 0; robot_id < state.robots.size(); ++robot_id) {
      if (state.robots[robot_id].request_id != NONE) {
        for (int request_id = 0; request_id < state.requests.size(); ++requests_id) {
  ...
  void GuidanceModel::getActionsAtState(const utexas_planning::State::ConstPtr& state_base,
    ...
    for (unsigned int idx_num = 0; idx_num < robot_ids.size(); ++idx_num) {
      int robot = robot_ids[idx_num].first;
      int request = robot_ids[idx_num].second;
      ...
      for (unsigned int adj = 0; adj < adjacent_vertices_map_[state.loc_node].size(); ++adj) {
        actions[action_counter] =
          Action::Ptr(new Action(DIRECT_PERSON, robot, adjacent_vertices_map_[state.loc_node][adj], request));
        actions[action_counter + 1] =
          Action::Ptr(new Action(LEAD_PERSON, robot, adjacent_vertices_map_[state.loc_node][adj], request));
        action_counter += 2;
      }
    }
  }
