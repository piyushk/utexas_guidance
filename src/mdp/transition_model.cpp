...
namespace utexas_guidance {
  ...
  int HumanDecisionModel::getNextNode(const State &state, RNG &rng) {
    ...
    if (state.assist_type == LEAD_PERSON) {
      ...
      if (!onSameFloor(state.assist_loc, state.loc_node)) {
        // Got directions to go to a different floor, pretty obvious.
      ...
      if (state.loc_prev != state.loc_node && onSameFloor(state.loc_prev, state.loc_node)) {
